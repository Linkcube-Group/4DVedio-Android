...
public class AudioRecorder {
	...
	public void startAudioRecorder(
			final ASmackRequestCallBack micSoundSetCallBack) {
		...
		Thread thread = new Thread() {
			...
			public void run() {
				...
				while (isRun) {
					...
					int v = 0;
					long sound = VoiceUtils.computeWaveLevel(buffer) * level;
					long waveng = VoiceUtils.computeFFTLevel(buffer) * level;
					// Log.i("AudioRecorderListener",
					// "sound:"+sound+"--waveng:"+waveng);
					...
					if (count == 1 && readdata != 0) {
						Timber.d("mic.speed:"
								+ String.valueOf((v / readdata) / 128));
						micSoundSetCallBack
								.responseSuccess((v / readdata) / 128);
					}
				}
			}
		};
